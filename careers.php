      ...
      if (json_last_error() !== JSON_ERROR_NONE) {
        ...
        if (empty($openings)) {
          echo '<p>No open positions at the moment. Please check back later.</p>';
            ...
            for ($i = 0; $i < min($items_per_page, count($openings)); $i++) {
              $job = $openings[$i];
...
HTML;
    ...
    <?php if (count($openings) > $items_per_page): ?>
    <div class="load-more-btn-projects py-4">
        <button class="btn btn-custom btn-custom-secondary" id="loadMoreProjects" onclick="loadMoreCareers()">
            Load More
        </button>
    </div>
    <?php endif; ?>
                ...
                <div class="row justify-content-center">
                    ...
                    <div class="col-12 col-md-6 col-lg-3">
                        <div class="custom-card">
                            ...
                            <button class="btn btn-custom btn-custom-secondary" data-service="Careers"
    ...
    const totalOpenings = <?php echo count($openings); ?>;
    ...
    function loadMoreCareers() {
        const container = document.getElementById('career-openings');
        const loadMoreBtn = document.getElementById('loadMoreProjects');

        let itemsToLoad = Math.min(loadMoreCount, totalOpenings - currentIndex);

        for (let i = 0; i < itemsToLoad; i++) {
            const openingIndex = currentIndex + i;
            const job = openings[openingIndex];
            ...
            const blogCard = `
                 <div class="job-posting">
                          <div class="row justify-content-start">
                            ...
                            <div class="job-body">
                                <p>${escapeHtml(job['roleDiscription'])}</p>
                            </div>
                            </div>
                            <div class="col-md-2">
                                  <a href="https://app.kapiree.com/info-candidate?id=${job['key']}"  target="_blank" class="btn btn-custom btn-custom-secondary">Apply Now</a>
